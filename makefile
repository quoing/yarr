...
build_macos:
	...
	GOOS=darwin GOARCH=$(TARGETARCH) go build -tags "sqlite_foreign_keys macos" -ldflags="$(GO_LDFLAGS)" -o _output/macos/yarr ./cmd/yarr
...
build_windows:
	...
	GOOS=windows GOARCH=$(TARGETARCH) go build -tags "sqlite_foreign_keys windows" -ldflags="$(GO_LDFLAGS) -H windowsgui" -o _output/windows/yarr.exe ./cmd/yarr
